...
object Anagrams extends AnagramsInterface:
  ...
  def wordOccurrences(w: Word): Occurrences = 
    w.toLowerCase.groupBy((elem:Char) => elem).map(a => (a._1, a._2.length)).toList.sorted
  ...
  lazy val dictionaryByOccurrences: Map[Occurrences, List[Word]] = 
    dictionary.groupBy((elem:Word) => wordOccurrences(elem))
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] = 
    ...
    ocs.foldLeft(List[Occurrences](Nil)){(l1,l2) => l1 ::: (for (elem1 <- l1; elem2 <- l2) yield elem1:::List(elem2)) }
