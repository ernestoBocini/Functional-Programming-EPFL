...
trait GameDef:
   ...
   */
  def startPos: Pos /* left abstract, we're going to give the value later*/

  /**
   * The target position where the block has to go.
   * This value is left abstract.
   */
  def goal: Pos /*same for goal*/
  ...
  case class Block(b1: Pos, b2: Pos):
    ...
    def neighbors: List[(Block, Move)] = 
      List((down, Move.Down), (up, Move.Up), (left, Move.Left), (right, Move.Right))
