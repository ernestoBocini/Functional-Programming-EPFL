...
trait EncoderInstances:
  ...
  given Encoder[String] =
    Encoder.fromFunction(s => Json.Str(String(s))) // TODO Implement the `Encoder[String]` instance
...
trait DecoderInstances:
  ...
  given [A] (using decoder: Decoder[A]): Decoder[List[A]] = 
    Decoder.fromFunction { 
      case  Json.Arr(items) => {
        val ls = items.map(decoder.decode) 
        if ls.forall(_.isDefined) then  Some(ls.map(_.get)) else None
      }
      case _ => None
    }
...
trait ContactsCodecs: 
    ...
    given Encoder[Contacts] = 
      ...
      //ObjectEncoder.field[List[String]]
